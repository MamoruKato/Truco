...
public:
	...
	//TODO BRENO OLHA DESSA LINHA PARA BAIXO
	
	virtual bool checa_fim_rodada(std::size_t _jog_atual);

	virtual std::size_t jogador_comeca();
	
	virtual void jogador_comeca(std::size_t _comeca);
	
	virtual std::size_t jogador_comeca_turno();
	
	virtual void jogador_comeca_turno(std::size_t _comeca_turno);
	
	virtual std::size_t jogador_termina();
	
	virtual void jogador_termina(std::size_t _termina);

	virtual int compara(Carta c1,Carta c2,Carta _vira);

	virtual void jogador_ganhou_rodada(std::size_t _jogador, std::size_t _rodada);

	virtual bool fim_turno(std::size_t _rodada);

	virtual std::size_t jogador_ganhou_turno(std::size_t _rodada);
